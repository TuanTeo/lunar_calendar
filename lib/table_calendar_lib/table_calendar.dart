...
class TableCalendarState<T> extends State<TableCalendar<T>> {
  ...
  late final ValueNotifier<DateTime> pFocusedDay;
  late RangeSelectionMode pRangeSelectionMode;
  DateTime? pFirstSelectedDay;

  @override
  void initState() {
    super.initState();
    pFocusedDay = ValueNotifier(widget.focusedDay);
  ...
  void didUpdateWidget(TableCalendar<T> oldWidget) {
    ...
    if (pFocusedDay.value != widget.focusedDay) {
      pFocusedDay.value = widget.focusedDay;
    }
  ...
  bool get _isRangeSelectionToggleable =>
      pRangeSelectionMode == RangeSelectionMode.toggledOn ||
      pRangeSelectionMode == RangeSelectionMode.toggledOff;

  bool get _isRangeSelectionOn =>
      pRangeSelectionMode == RangeSelectionMode.toggledOn ||
      pRangeSelectionMode == RangeSelectionMode.enforced;
  ...
  void _onDayTapped(DateTime day) {
    final isOutside = day.month != pFocusedDay.value.month;
    ...
    if (_isRangeSelectionOn && widget.onRangeSelected != null) {
      if (pFirstSelectedDay == null) {
        pFirstSelectedDay = day;
        widget.onRangeSelected!(pFirstSelectedDay, null, pFocusedDay.value);
      } else {
        if (day.isAfter(pFirstSelectedDay!)) {
          widget.onRangeSelected!(pFirstSelectedDay, day, pFocusedDay.value);
          pFirstSelectedDay = null;
        } else if (day.isBefore(pFirstSelectedDay!)) {
          widget.onRangeSelected!(day, pFirstSelectedDay, pFocusedDay.value);
          pFirstSelectedDay = null;
        }
      }
    } else {
      widget.onDaySelected?.call(day, pFocusedDay.value);
    }
  }
  ...
  void _onDayLongPressed(DateTime day) {
    ...
    if (widget.onDayLongPressed != null) {
      _updateFocusOnTap(day);
      return widget.onDayLongPressed!(day, pFocusedDay.value);
    }
  ...
  void _updateFocusOnTap(DateTime day) {
    if (widget.pageJumpingEnabled) {
      pFocusedDay.value = day;
      return;
    }

    if (widget.calendarFormat == CalendarFormat.month) {
      if (_isBeforeMonth(day, pFocusedDay.value)) {
        pFocusedDay.value = _firstDayOfMonth(pFocusedDay.value);
      } else if (_isAfterMonth(day, pFocusedDay.value)) {
        pFocusedDay.value = _lastDayOfMonth(pFocusedDay.value);
      } else {
        pFocusedDay.value = day;
      }
    } else {
  ...
  void _toggleRangeSelection() {
    if (pRangeSelectionMode == RangeSelectionMode.toggledOn) {
      pRangeSelectionMode = RangeSelectionMode.toggledOff;
    } else {
      pRangeSelectionMode = RangeSelectionMode.toggledOn;
    }
